...
class Round_Button(tk.Label):
    ...
    def create_custom_image(self):
        ...
        while True:
            ...
            decrement += 1
            font = ImageFont.truetype('gui/styles/Assets/roboto/Roboto-Regular.ttf',
                                      int(4 * self.multi) - decrement, encoding="unic")
            coords, Lines, line_height = self.draw_multiple_line_text(
                self.text, font, int(36 * self.multi), int(2.2 * self.multi), 12)
        ...
        self.image_colours = [
            [self.tc[i] + ((self.sc[i]-self.tc[i])//10)*x for i in range(3)] for x in range(10)]
        self.text_colours = [
            [self.ttc[i] + ((self.tsc[i] - self.ttc[i]) // 10) * x for i in range(3)] for x in range(10)]
        self.outline_colours = [[self.trans_outline[i] + (
            (self.static_outline[i] - self.trans_outline[i]) // 10) * x for i in range(3)] for x in range(10)]
        for i in range(10):
            ...
            self.image_drawer[i].rectangle((int(5.5 * self.multi), 0, self.resoltuion[0] - int(
                5.5 * self.multi), self.resoltuion[1]-1), outline=outline, width=2, fill=colour)
            ...
            self.image_drawer[i].rectangle(
                (self.resoltuion[0] - int(5.5 * self.multi), 0, self.resoltuion[0], self.resoltuion[1]-2), fill=(0, 0, 0, 0))
            self.image_drawer[i].ellipse((self.resoltuion[0] - int(10 * self.multi), 0,
                                          self.resoltuion[0]-1, self.resoltuion[1]-2), outline=outline, width=2, fill=colour)

            self.image_drawer[i].rectangle(
                (0, 0, int(5.5 * self.multi), int(10 * self.multi)-2), fill=(0, 0, 0, 0))
            self.image_drawer[i].ellipse(
                (0, 0, int(10 * self.multi), int(10 * self.multi)-2), outline=outline, width=2, fill=(colour))

            self.image_drawer[i].rectangle(
                (int(5.5 * self.multi), 2, self.resoltuion[0] - int(5.5 * self.multi), self.resoltuion[1]-3), fill=colour)

            for x in range(len(coords)):
                self.image_drawer[i].text(
                    coords[x], Lines[x], fill=textcolour, font=font, align='center')
    ...
    def create_lower_button(self):
        ...
        while True:
            ...
            font = ImageFont.truetype('gui/styles/Assets/Roboto-Regular.ttf',
                                      int(4 * multi) - decrement, encoding="unic")
            coords, Lines, line_height = self.draw_multiple_line_text(
        ...
        self.lower_drawer.rectangle(
            (0, 0, resoltuion[0], resoltuion[1]-1), outline=outline, width=2,  fill=colour)
        ...
        # Right side
        self.lower_drawer.rectangle(
            (resoltuion[0] - int(5.5*multi), 0, resoltuion[0], resoltuion[1]), fill=(0, 0, 0, 0))
        self.lower_drawer.ellipse(
            (resoltuion[0] - int(10*multi), 0, resoltuion[0], resoltuion[1]), outline=outline, width=2, fill=colour)
        ...
        self.lower_drawer.rectangle(
            (int(5.5 * multi), 2, resoltuion[0] - int(5.5*multi), resoltuion[1]-3), fill=colour)
        ...
        self.lower_button = self.lower_button.resize(
            size=(self.resoltuion[0] - delta_x*2, self.resoltuion[1] - delta_y*2))
        ...
        self.Button.paste(self.lower_button, (delta_x, delta_y,
                                              self.resoltuion[0] - delta_x, self.resoltuion[1] - delta_y), self.lower_button)
...
if __name__ == '__main__':
    ...
    Button = Round_Button(app, 'Login', 3, Static_Colour, Text_Static_Colour, Transformation_Colour,
                          Text_Transformation_Colour, Background, Static_Outline, Transformation_Outline)
