...
def pytest_addoption(parser):
    ...
    parser.addoption("--rules", action = "store", default = "00", 
                     help = "choose active rules for generator in form of binary string")
...
def pytest_generate_tests(metafunc):
    if "params" in metafunc.fixturenames:
        ...
        metafunc.parametrize("params", params)#((m, n, c, list_for_solver),))
    ...
    for i in range(2, n - 2):
        for j in range(2, m - 2):
            if random.randint(0, 1):
                ...
                for a in range(-2, 3):
                    for b in range(-2, 3):
                        if (abs(a * b) == 1 or a * b  == 0) and board[i+a][j+b] != -1:
                ...
                if row_c[i - 2][color] > 0 and col_c[j - 2][color] > 0:
                    #print (poss)
                    poss = poss - ({board[i-2][j]} & {board[i-1][j]}) - ({board[i-1][j]} & {board[i+1][j]}) - \
                        ({board[i+1][j]} & {board[i+2][j]}) - ({board[i][j-2]} & {board[i][j-1]}) - \
                        ({board[i][j-1]} & {board[i][j+1]}) - ({board[i][j+1]} & {board[i][j+2]})
                    if len(poss) == 0:
                        continue
                    else:
                        board[i][j] = color
